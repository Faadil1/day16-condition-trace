    ...
    <div className={`comparison-visual ${raking ? 'is-raking' : 'is-diffuse'}`} aria-hidden="true">
      <svg className="evidence-vessel-plate" viewBox="0 0 260 150" role="presentation">
        <defs>
          ...
          <filter id={`soft-${suffix}`} x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="3" />
          </filter>
        </defs>

        <ellipse cx="130" cy="137" rx="54" ry="6" fill="#050302" opacity=".45" filter={`url(#soft-${suffix})`} />
        ...
        {[96, 106, 116, 126, 136, 146, 156, 166].map((x, index) => (
          <path
            key={x}
            d={`M${x} 98 C${x + (index % 2 ? 4 : -3)} 105 ${x + (index % 2 ? -2 : 3)} 113 ${x} 122`}
            fill="none"
            stroke="#625b42"
            strokeWidth="2.2"
            strokeLinecap="round"
        ...
        <path
          d="M75 52 C68 48 61 51 57 58 C51 69 53 88 63 97 C67 101 72 103 78 102"
          fill="none"
          stroke="#b29d72"
          strokeWidth="5"
          strokeLinecap="round"
        ...
        />
        <path
          d="M185 52 C192 48 199 51 203 58 C209 69 207 88 197 97 C193 101 188 103 182 102"
          fill="none"
          stroke="#b29d72"
          strokeWidth="5"
          strokeLinecap="round"
        ...
        {raking && (
          <>
            <path d="M154 39 L176 137" stroke="#f5e6bd" strokeWidth="20" opacity=".12" filter={`url(#soft-${suffix})`} />
            <path d="M157 47 C160 52 157 56 162 61 C158 66 163 71 160 78" fill="none" stroke="#2e261e" strokeWidth="1.7" strokeLinecap="round" opacity={Math.max(.45, reveal)} />
            <circle cx="160" cy="61" r="15" fill="none" stroke="#b58a59" strokeDasharray="2 4" opacity=".38" />
          </>
        )}
      </svg>
