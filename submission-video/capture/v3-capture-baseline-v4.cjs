/*
 * Condition Trace V3 — deterministic submission capture baseline v4
 *
 * Captures only the real V3 application. No proof-bearing UI is recreated.
 * A MutationObserver is armed before Capture observation so the short
 * OBS-04 preservation seal is verified even when canvas capture blocks
 * Playwright long enough for the transient to disappear before control returns.
 */
'use strict';
...
async function armTransientObserver(page) {
  await page.evaluate(() => {
    const audit = { framePreserved: false, humanReviewed: false, obs04: false };
    window.__conditionTraceCaptureAudit = audit;
    const scan = () => {
      const text = document.body.innerText;
      if (text.includes('FRAME PRESERVED')) audit.framePreserved = true;
      if (text.includes('Human-reviewed observation captured')) audit.humanReviewed = true;
      if (text.includes('OBS-04')) audit.obs04 = true;
    };
    scan();
    const observer = new MutationObserver(scan);
    observer.observe(document.body, { subtree: true, childList: true, characterData: true });
    window.__conditionTraceCaptureObserver = observer;
  });
}

async function readTransientObserver(page) {
  return page.evaluate(() => {
    const audit = window.__conditionTraceCaptureAudit || {};
    if (window.__conditionTraceCaptureObserver) window.__conditionTraceCaptureObserver.disconnect();
    return {
      framePreserved: Boolean(audit.framePreserved),
      humanReviewed: Boolean(audit.humanReviewed),
      obs04: Boolean(audit.obs04),
    };
  });
}

async function triggerObservation(page, capturePostState = null) {
  await armTransientObserver(page);
  const button = page.getByRole('button', { name: /Capture observation/i });
  await button.evaluate(el => el.click());
  await sleep(40);

  const transient = await readTransientObserver(page);
  note('OBS-04 transient mounted', transient.obs04);
  note('OBS-04 frame-preserved transient observed', transient.framePreserved);
  note('OBS-04 human-reviewed transient observed', transient.humanReviewed);

  if (capturePostState) await capturePostState();

  await assertStep(page, 'compare');
  await bodyHas(page, 'OBS-04', 'OBS-04 carried into comparison state');
}
...
async function primaryCapture(browser) {
  ...
  await triggerObservation(page, async () => {
    await capture(page, '04-condition-trace-v3-post-capture-16x9.png', 'OBS-04 / real post-capture application state', viewport);
  });
...
(async () => {
  console.log(`[baseline-v4] URL=${URL}`);
  console.log(`[baseline-v4] ref=${SOURCE_REF}`);
  console.log(`[baseline-v4] sha=${SOURCE_SHA}`);
  ...
  try {
    ...
    console.log(`[baseline-v4] report written; passed=${report.passed}`);
  }

  if (!report.passed) process.exit(1);
  console.log('[baseline-v4] PASS — V3 capture baseline complete.');
})().catch(error => {
  console.error(error);
  process.exit(1);
});
