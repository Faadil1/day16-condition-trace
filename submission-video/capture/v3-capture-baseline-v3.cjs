/*
 * Condition Trace V3 — deterministic submission capture baseline v3
 *
 * Captures only the real V3 application. No proof-bearing UI is recreated.
 * The short OBS-04 preservation seal is asserted synchronously before the
 * full-frame screenshot, because WebGL screenshots can take longer than the seal.
 */
'use strict';
...
async function triggerObservation(page, capturePostState = null) {
  ...
  // Validate the real transient seal before any expensive full-frame screenshot.
  await page.waitForFunction(() => document.body.innerText.includes('FRAME PRESERVED'), null, { timeout: 700 });
  note('OBS-04 frame-preserved transient observed', true);
  const humanReviewed = await page.evaluate(() => document.body.innerText.includes('Human-reviewed observation captured'));
  note('OBS-04 human-reviewed transient observed', humanReviewed);

  // A 1920x1080 WebGL screenshot may finish after the 430ms seal disappears.
  // This still captures the real post-capture application state; it is not used
  // as proof that the transient seal lasted longer than it actually does.
...
async function primaryCapture(browser) {
  ...
  await triggerObservation(page, async () => {
    await capture(page, '04-condition-trace-v3-post-capture-16x9.png', 'OBS-04 / real post-capture transition', viewport);
  });
...
(async () => {
  console.log(`[baseline-v3] URL=${URL}`);
  console.log(`[baseline-v3] ref=${SOURCE_REF}`);
  console.log(`[baseline-v3] sha=${SOURCE_SHA}`);
  ...
  try {
    ...
    console.log(`[baseline-v3] report written; passed=${report.passed}`);
  }

  if (!report.passed) process.exit(1);
  console.log('[baseline-v3] PASS — V3 capture baseline complete.');
})().catch(error => {
  console.error(error);
  process.exit(1);
});
