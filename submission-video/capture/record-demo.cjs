...
const OUT_FILE = path.join(OUT_DIR, 'canonical-v3-live-recording-v2.webm');
...
(async () => {
  ...
  await clickText(page, 'BEGIN RETURN INSPECTION');
  await WAIT(20000);
  ...
  const files = fs.readdirSync(OUT_DIR).filter(f => f.endsWith('.webm') && f !== 'canonical-v3-live-recording-v2.webm');
